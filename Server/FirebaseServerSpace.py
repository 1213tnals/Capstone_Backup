...
def download_video_space():
    ...
    # 다운로드 경로 설정
    download_path = "../../gaussian-splatting/data/test2/input/"
...
def process_video_space(file_extension):
	destination_filename = f"../../gaussian-splatting/data/test2/input/input_video.{file_extension}"
    # 동영상 파일일 경우 FFmpeg 명령어 실행
	if file_extension == "mp4":
		ffmpeg_command = [
        "ffmpeg", "-i", destination_filename, "-r", "0.25", "-q:v", "2", 
        "-start_number", "0", "../../gaussian-splatting/data/test2/input/%03d.jpg"
        ]
