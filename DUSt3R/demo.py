...
#### 받은 데이터들을 통해 Scene에다가 초기 포인트 클라우드를 추가 및 생성
def _convert_scene_output_to_ply(outdir, imgs, pts3d, mask, focals, cams2world, cam_size=0.05,
                                 cam_color=None, as_pointcloud=True,
                                 transparent_cams=True, silent=False):
    ...
    # full pointcloud
    if as_pointcloud:
        pts = np.concatenate([p[m] for p, m in zip(pts3d, mask)])                               # information of Points
        col = np.concatenate([p[m] for p, m in zip(imgs, mask)])                                # information of Colors
        pct = trimesh.PointCloud(pts.reshape(-1, 3), colors=col.reshape(-1, 3), size=0.05)      # PointCloud에 담음
        scene.add_geometry(pct)                                                                 # Scene에다가 추가
    else:
        meshes = []
    ...
    # add each camera         # 나는 맵만을 뽑을 예정이므로 이 부분을 사용하지 않음
    for i, pose_c2w in enumerate(cams2world):
        ...
        ####### 새로운 내용 추가함 #######
        # pose_c2w와 focals[i] 데이터를 텍스트 파일에 저장
        with open('./suminEdit/poses_and_focals.txt', 'a') as file:
            np.savetxt(file, pose_c2w, header=f'Camera {i} pose_c2w:', comments='')
            file.write(f'focal_length: {focals[i]}\n\n')
        ##############################
    ...
    outfile = os.path.join(outdir2, 'scene.ply')
    if not silent:
        print('(exporting 3D scene to', outfile, ')')
    scene.export(file_obj=outfile)
    # scene.export('./suminEdit/scene.ply')
    return outfile
...
def get_3D_model_from_scene(outdir, silent, scene, min_conf_thr=3, as_pointcloud=True, mask_sky=False,      # 206번줄에서 scene을 정의
    ...
    return _convert_scene_output_to_ply(outdir, rgbimg, pts3d, msk, focals, cams2world, as_pointcloud=as_pointcloud,
                                        transparent_cams=transparent_cams, cam_size=cam_size, silent=silent)
...
if __name__ == '__main__':
    ...
    if args.tmp_dir is not None:              # if there are no folder, make it
        tmp_path = args.tmp_dir
        os.makedirs(tmp_path, exist_ok=True)
        tempfile.tempdir = tmp_path

    if args.server_name is not None:          # get server address
        server_name = args.server_name
    else:
        server_name = '0.0.0.0' if args.local_network else '127.0.0.1'

    if args.weights is not None:              # get address
    ...
    #### main_demo 함수를 작동 ####
    with tempfile.TemporaryDirectory(suffix='dust3r_gradio_demo') as tmpdirname:      # 임시 폴더를 만들었다가 삭제함
        if not args.silent:
