...
# __init__.py의 global_aligner()가 있는 net = PointCloudOptimizer()에서 반환받고,
# optimizer.py의 클래스는 PointCloudOptimizer(BasePCOptimizer)여서 BasePCOptimizer의 구조를 상속받음
# 이때, BasePCOptimizer 클래스 내부에서 self.imgs, get_focals() 등이 있고, 함수는 PointCloudOptimizer에서 정의됨
imgs = scene.imgs
...
# 5. Make PLY file
H,W = pts3d[0].shape[:2]    # pts3d[0] = (height, width, 3). pts3d[0].shape[:2] = 텐서의 1,2번째 차원의 크기 = 이미지의 높이(H)와 너비(W)
pcd_array = pts3d[0].view(H*W, 3).detach().cpu().numpy()    # (height, width, 3) -> (height * width, 3). 각 행이 하나의 3D 포인트(3차원 좌표)를 나타냄.
